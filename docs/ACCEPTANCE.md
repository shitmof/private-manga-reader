...
- `flutter analyze`：零问题；`flutter test`：48 项全部通过。
- 本地与挂载阅读器共享白蓝顶部栏、页码胶囊和纵向定位器；默认画纸与可选夜间偏好已持久化。
- 阅读专用路由使用 240ms 白底淡入缩放；书单层级使用 280ms 淡入缩放，并尊重 Android 减少动态效果。
- 360dp 基准书架为三列、16dp 左右留白、14dp 列距、24dp 行距；漫画与白色四宫格书单使用相同高度节奏。
- 拖动反馈保持源卡片实际尺寸并抬升；合组目标轻微放大，不再剧烈缩小；新建/加入书单均使用白色底部确认面板。

### Android 覆盖升级与数据留存

- 使用 `adb install -r` 将阴阳师模拟器中的正式 V1.4.0+5 覆盖升级为 V1.5.0+6，未卸载、未清数据。
- `firstInstallTime` 保持 `2026-08-30 21:32:01`；包名与签名证书保持不变。
- SQLite `user_version` 从 7 升至 8，并生成 167,936 字节的 `library.db.pre-v8`。
- 升级前后均为 3 本漫画、2 个书单、3 个根书架条目、1 个挂载源、2 本挂载漫画、5 个远程页面。
- 挂载漫画总页数保持 5，`MountedDemo` 阅读位置保持索引 2（界面第 3 张）。

### ADB 阅读交互闭环

- 打开挂载漫画后，空闲第一帧为白色画布，顶部控制栏和右侧滑轮均不占用画面。
- 点按中央后显示白色顶部栏、蓝色返回/更多按钮和白底蓝字 `3 / 3` 页码胶囊。
- 在右侧隐形热区从下向上拖动，蓝色纵向定位器出现并将进度从 `3 / 3` 直接定位到 `1 / 3`。
- 松手约 1.1 秒后，轨道和滑块再次完全隐藏，漫画布局没有位移。
- 设置页显示独立“阅读画布：画纸 / 夜间”，默认选中画纸。
...
- APK Signature Scheme v2 校验通过；签名证书 SHA-256 为 `3724690e26bae1597ad10221c10f2b2b15ade2a5bfdbaf408834dc46eaace1a5`，与 V1.0～V1.4 一致。

### 公开前审计与边界

- 当前分支与完整 Git 历史未发现 `.env`、密钥、签名库、数据库、`.mangabackup`、私人漫画或用户照片。
- Git 中的压缩包仅为自动化合成测试夹具；PNG 仅为应用图标、设计参考和系统模板资源。
- 书单逐格 Hero 与完整物理弹簧重排尚未实现，已在 V1.5 发布说明中明确列为后续增强，不冒充完成。
