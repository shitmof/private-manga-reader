...
GitHub 仓库是源码与安装包的远端回退点；手机内漫画数据通过 `.mangabackup` 独立备份。两者不要混用。
...
```powershell
git tag -a v1.5.0 -m "Shihuage v1.5.0"
...
- `backup-pre-v1.5-20260831`：白蓝阅读器、动态白色书单与统一动效实施前的 V1.4 稳定点。
- `v1.5.0`：白蓝阅读器、独立夜间偏好、三列卡片节奏、动态白色书单和合组确认面板正式版。
...
升级 APK 不会移动或重编码 `assets/`。数据库首次升级到 schema 6、7、8 前分别会自动保留 `library.db.pre-v6`、`library.db.pre-v7`、`library.db.pre-v8`；完整书架的数据回退仍以升级前导出的 `.mangabackup` 为准。SAF 挂载的原文件始终位于用户选择的外部目录，恢复备份后需要重新选择该目录恢复授权。
