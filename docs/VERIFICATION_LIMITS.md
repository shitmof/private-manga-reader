...
**结论**：`flutter test` 默认并行执行时，
`library_screen_test.dart` 的「书架使用三列、文件夹分组与验证后私密区」
会偶发失败；改为 `flutter test --concurrency 1` 后连续通过。

**排查过程**（三轮，均已做过诊断打印，非推断）：

1. 起初归因为「磁盘写入慢」，把等待从 500ms 改为 3 秒、又改为 20 秒轮询，
   均未解决——说明不是等待时长问题。
2. 找到第二个诱因：书架用 `AnimatedSwitcher` 承载作用域内容，
   过渡期间**同时保留新旧子树**，此时 `find.text` 仍能找到正在淡出的旧卡片。
   改用 `pumpAndSettle()` 等过渡结束后，并行下仍偶发失败。
3. 在断言点打印实际状态确认：`becamePrivate=true`、`nowPrivate=true`，
   渲染内容中已无目标文本。即**应用行为正确**，失败来自测试并发环境。

**可靠运行方式**：`flutter test --concurrency 1`。

**未解决**：并行下的具体干扰源未定位。已排除等待时长与切换动画两个因素，
推测与并行隔离或资源竞争有关，未继续深挖。
