...
- 这是**唯一渠道**，本版使用与 **v1.4.0 / v1.5.0 相同**的密钥，
  **已装 v1.4.0 或 v1.5.0 可直接覆盖升级、保留数据，无需卸载**。

> ⚠️ **已装 v1.6.0 / v1.6.1 的用户请注意**：这两版误用了另一把密钥
> （`1F6A95C4…`），安装 v1.6.2 会提示「软件包与现有软件包存在冲突」。
> 需要先卸载——会清除应用内数据，请先在「设置」中导出备份，装好后再恢复。
> 这是历史遗留问题：应用签名一旦发布便无法更改。v1.6.2 已改回正确密钥，
> 后续版本不会再出现此问题。
